  ...
  try {
    ...
    switch (method) {
      ...
      case 'PUT':
        ...
        const putUserId = getUserIdFromUrl(url);
        if (!putUserId) {
          sendError(res, 'Not found', 404);
          return;
        }

        if (!isValidUUID(putUserId)) {
          sendError(res, 'Invalid user id format', 400);
          return;
        }

        if (!users[putUserId as UUID]) {
          sendError(res, 'User not found', 404);
          return;
        }
        ...
        try {
          ...
          users[putUserId as UUID] = {
            id: putUserId as UUID,
            ...body,
          };

          sendData(res, users[putUserId as UUID]);
        } catch {
          sendError(res, 'Invalid JSON', 400);
        }
        return;
